...
(function() {
  ...
  function createPackageNormalize(normalize, sync) {
    return function(name, parentName, isPlugin) {
      ...
      // strip package conditional syntax for bundle support
      var bundledConditional = name.indexOf('#:') != -1;
      if (bundledConditional)
        name = name.replace('#:', '/');
      ...
      // if the normalized package path is already defined, then skip configuration loading
      var alreadyConfigured = pkgName && (bundledConditional || normalized in loader.defined || loader.has[normalized]);
